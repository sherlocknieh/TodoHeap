		...
		<nav class="bg-white border-b border-slate-200">
			<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div class="flex gap-1 overflow-x-auto">
					<button v-for="view in ['list', 'tree', 'heap']" :key="view" @click="switchView(view)" :class="[
		...
		<main class="flex-1 overflow-hidden bg-slate-50">
			<div class="max-w-7xl mx-auto h-full px-4 sm:px-6 lg:px-8">
				<div class="flex gap-4 h-full py-4">
					<!-- 左侧：三视图内容 -->
					<div
						class="flex-1 min-w-0 bg-white rounded-lg shadow-sm border border-slate-200 flex flex-col overflow-hidden">
						<!-- 移动端 AI 分解按钮 -->
						...
						<div class="flex-1 overflow-auto p-4 sm:p-6">
							...
							<div v-else-if="activeView === 'tree'">
								<div v-if="todoStore.loading"
									class="flex items-center justify-center h-96 text-slate-500">
									<div class="text-center">
										<p class="text-lg mb-2">⏳</p>
										<p class="text-sm">加载中...</p>
									</div>
								</div>
								<TodoTree v-else :selected-task-id="selectedTaskId"
